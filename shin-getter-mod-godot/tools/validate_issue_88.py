...
# Catalog expectations superseded by issue#214; UI/lifecycle checks remain unchanged.
_MANIFEST = json.loads((ROOT / "tools/issue214-bgm-manifest.json").read_text(encoding="utf-8"))
TRACK_TITLES = {
    "DEFAULT": ("（预设）", "（標準）", "(default)"),
    **{t["id"].upper(): tuple(t["titles"][lang] for lang in LANGUAGES) for t in _MANIFEST["tracks"]},
    "RANDOM": ("随机", "ランダム", "Random"),
}
ALBUM_FILES = {Path(t["resource"]).name for t in _MANIFEST["tracks"] if "/album/" in t["resource"]}
...
def validate_catalog_and_assets() -> None:
    ...
    if catalog.index("Track(RandomTrackId") < catalog.index('Track("bravery_iron_saga"'):
